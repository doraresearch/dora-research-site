...
- Homepage must immediately communicate that DORA operates above the model layer through application, orchestration, and harness layers; the How-DORA-works section (`#how-it-works`) is an interactive accordion + dark visual panel with the Aurora-ribbon canvas behind per-step operational vignettes (2026-06-12); footer closes on the giant Aurora "DORA" wordmark.
