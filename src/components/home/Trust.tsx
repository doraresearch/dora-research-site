...
export default function Trust() {
  return (
    <section id="control" className="relative bg-white py-24 sm:py-32">
      ...
      <Container>
        <Reveal>
          <div className="relative rounded-stage border border-white/[0.10] bg-graphite p-8 sm:p-12 lg:p-16">
            {['top-4 left-4', 'top-4 right-4 rotate-90', 'bottom-4 right-4 rotate-180', 'bottom-4 left-4 -rotate-90'].map((pos) => (
              <span key={pos} className={`pointer-events-none absolute ${pos} h-4 w-4 border-l-[1.5px] border-t-[1.5px] border-white/[0.15]`} />
            ))}
            ...
            <div className="relative">
              ...
              <h2 className="mt-4 max-w-[26ch] text-[32px] font-bold leading-[1.04] tracking-[-0.03em] text-white/90 sm:text-[42px]">
                ...
                {safeguards.map((s, i) => (
                  <Reveal key={s.title} delay={i * 60}>
                    <div className="py-3">
                      ...
                      <p className="mt-1.5 text-[14px] leading-[1.55] text-white/60">{s.description}</p>
                    </div>
                  </Reveal>
                ))}
              </div>
